...
void handleLogin(const crow::request& req, crow::response& res)
{
    ...
    if(RedisManager::block_user(email,IP))
    {
        try
        {
            ...
            int Attempt_status = RedisManager::Attempt_check(email,IP,password);
    ...
    else
    {
        ...
        res.code= 429;
        res.body = "{\"status\":\"error\",\"error\":\"Attempt Limit Exceed.Please Try again later!\"}";
    }
}